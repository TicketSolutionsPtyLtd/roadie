import { createElement } from 'react'

import { renderToString } from 'react-dom/server'
import { describe, expect, it } from 'vitest'

import { Funnel } from '.'
import { renderChartSvg } from '../static'
import { funnel } from './definition'
import { checkoutExample, waitlistExample } from './examples'

// Focus layers exist only in the live chart; the static file drops them.
const keysOf = (markup: string) =>
  [...markup.matchAll(/data-ts-key="([^"]+)"/g)]
    .map((m) => m[1]!)
    .filter((key) => !/^(default-focus|focus-guide-layer)/.test(key))
    .sort()

describe('Funnel static output', () => {
  it.each(['light', 'dark'] as const)(
    'matches the %s snapshot',
    async (mode) => {
      await expect(
        renderChartSvg(funnel, checkoutExample, {
          mode,
          width: 640,
          height: 220
        })
      ).toMatchFileSnapshot(`./__snapshots__/checkout-${mode}.svg`)
    }
  )

  it.each([
    ['checkout', checkoutExample],
    ['waitlist', waitlistExample]
  ])('draws the same marks as React for %s', (_, props) => {
    expect(keysOf(renderToString(createElement(Funnel, props)))).toEqual(
      keysOf(
        renderChartSvg(funnel, props, {
          mode: 'light',
          width: 640,
          height: 220
        })
      )
    )
  })
})
