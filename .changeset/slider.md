...
text, and `invalid` and `disabled` from there. The fill takes the inherited
intent, like `Progress`, and each thumb has a 44px touch target. The parts
(`Label`, `Value`, `Control`, `Track`, `Indicator`, `Thumb`) compose for
custom layouts such as tick marks.
