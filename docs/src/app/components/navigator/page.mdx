...
`useNavigatorSecondary` do nothing. Write each rail as a function you call, not a
...
`overview` on `Navigator.Secondary` shows your page alone on the section's
              ...
              <Pane>
                ...
                {path === '/' ? (
                  <div className='grid gap-3 pb-4'>
                    <p className='text-subtle'>
                      The root shows the page alone. Navigator.SecondaryItems
...
- `overview` needs the section's `href`. Without one, Navigator warns and
  shows the list.
...
`Navigator.SecondaryItems` renders a section's items as a grouped `List`, the same
...
For your own layout, `useNavigatorSecondary(value?)` returns the same data. Loose
items come back as a group with no title.

```tsx
function HomeCards() {
  const section = useNavigatorSecondary('/')
...
before your detail pane. `<Navigator.SecondaryItems showDescriptions={false} />`
...
```tsx
const { start, stop } = usePendingNavigation()
...
<Guideline title='Overview or list'>
  <Guideline.Do code={`<Navigator.Item value='/' href='/' icon={<HouseIcon />}>\n  Home\n  <Navigator.Secondary aria-label='Home pages' overview>…</Navigator.Secondary>\n</Navigator.Item>`}>
    Use `overview` when the section has a page worth reading, such as
  ...
  <Guideline.Dont code={`<Navigator.Item value='/inbox' href='/inbox' icon={<TrayIcon />}>\n  Inbox\n  <Navigator.Secondary aria-label='Messages' overview searchable>…</Navigator.Secondary>\n</Navigator.Item>`}>
    Give `overview` to a section whose route would only say "pick one". An
