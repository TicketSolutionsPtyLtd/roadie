...
- Navigator is controlled. The active `value` comes from your router. The
  ...
  Matching is exact, plus a `value + '/'` prefix that marks an item active for
  the pages under it, so normalise a pathname before passing it. A trailing
  slash misses the exact match and lands on the prefix one, and the item reads
  as one level deeper than you are: `/events/` looks like a page below
  `/events` rather than `/events` itself.
...
### Panes in route segments

A shell usually draws its panes from nested route layouts, each segment adding
one. That works, and it decides what motion you get: a push slides the arriving
pane in over the one going behind, and a pop slides the pane behind back as the
one above it is removed. The pane being left is not animated out, because the
router has already removed its elements and nothing Roadie holds can draw them
again. `Pane` has the measurements under
[Route-driven shells](/components/pane#route-driven-shells).

`value` has to come from the same route that renders the panes, such as
`usePathname()`, or the stack reads the wrong depth.
...
- A step between the root and a sub-page slides, from a copy of the page being
  left: one pane draws both, so there is nothing else to move. `value` has to
  come from the route that renders the page, such as `usePathname()`, or the
  step, the list pane and Back all read the wrong route.
...
Tap a link and the router may take a while. After 150ms with nothing on screen
changed, the frame fills with a slowly turning gradient of three Oztix colours,
behind the panes and the nav's capsules, and on a phone the panes pull back a
little and round their corners to show it. It runs until the destination
arrives, then fades. Anything faster than 150ms shows nothing at all, so a
prefetched route stays quiet.

Nothing to wire: `RoadieLinkProvider` marks a plain left click on the internal
`href` of any Roadie surface that takes one, and clears it when the destination
lands. Turn it off with `pendingIndicator={false}` on the provider.

```tsx
<RoadieLinkProvider Link={NextLink} pendingIndicator={false}>
```

Two cases Roadie cannot see, and what to do about each.

A route with a `loading.tsx` commits as soon as its skeletons are on screen, so
the indicator stops there while the real content is still coming. Pass
`pending` on the loading state's pane to hold it until the page lands:
...
A navigation Roadie never saw is not marked at all: one you start with
`router.push`, or a link that is not a Roadie surface, such as a bare
`next/link`. Report those:

```tsx
const { start, stop } = useReportPendingNavigation()
...
The route landing calls `stop` for you, so it is only needed on the path where
the navigation never happens. Without it a failed attempt runs to the ceiling.

A mounted `Skeleton` is deliberately not a signal on its own. A skeleton is as
likely to be a placeholder that never resolves, or a demo of the component
itself, so it never starts or holds the indicator. `pending` is the one place
that says a wait is real.

The indicator ends on the first of the destination landing, a Back or Forward
traversal, the tab going to the background, and a 10 second ceiling, which also
catches a pane left with `pending` and logs it in development. A pane still
reporting `pending` outlasts the first three, because a Back press does not make
its content arrive.

Reduced motion keeps the colour and drops both the turn and the phone
pull-back. The indicator announces nothing: it is `aria-hidden`, a pane with
...
- An item always links to its declared `href`, in More too, so tapping a
  top-level item goes to that section's root wherever you were inside it.
