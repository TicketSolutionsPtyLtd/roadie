...
- A step between the root and a sub-page cuts: one pane draws both, so there is
  nothing to move. `value` still has to come from the route that renders the
  page, such as `usePathname()`, or the list pane and Back go to the wrong place.
