...
Nothing to wire: `RoadieLinkProvider` marks a plain left click on an internal
`href`, wherever it came from, and `Navigator` clears it when the destination
...
A navigation you start yourself, with `router.push` in a handler, is not a
click Roadie saw. Report it:
...
const buy = async () => {
  start()
  await reserve()
  router.push('/checkout') // `stop()` is not needed; the route landing clears it
}
```
...
traversal, the tab going to the background, and a 10 second ceiling for a
navigation that never arrives. Reduced motion keeps the colour and drops the
turn. The frame carries `aria-busy` while it runs, and nothing is announced.
