...
- Author the whole tree in a client component. Navigator finds its parts by
  ...
  items, group titles and menus disappear. `Pane` is server-safe.
...
- Everything else you pass to `Navigator` is its content, and Navigator wraps
  it in `Navigator.Content` for you. Write `Navigator.Content` yourself only to
  pass props to the `<main>` it renders.

### Splitting a big tree

Moving each rail into its own `UserRail` component hides `Navigator.Primary`
...
- `value` has to come from the route that renders the page, such as
  `usePathname()`, or the step between pages, the list pane and Back all read
  the wrong route.
...
rather than replace, so the browser's Back undoes the change. Build the new
query from `window.location.search`, so the page's own params survive the flag.
...
`pending` on a [`Pane`](/components/pane) is the manual override. It holds the
indicator for a wait Roadie can't see, such as a client fetch without Suspense
or a mutation. A mounted `Skeleton` never starts or holds it, because a
...
- An item always links to its declared `href`, in More too, so tapping a
  top-level item goes to that destination's route from any of its pages.
