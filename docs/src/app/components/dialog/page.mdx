...
Each example leads with an [IconTile](/components/icon-tile) (`size='2xl'`, its `intent` inherited from the dialog) centred above the title — `Dialog.Header` handles the centring.
...
<Guideline title='Lead notable dialogs with a matching icon'>
  <Guideline.Do>
    For confirmations and notable moments, centre a `size='2xl'` [IconTile](/components/icon-tile) above the title and leave its `intent` unset so it inherits the dialog's — a danger or warning tile for caution, success for completion. It signals the nature of the action before the title is read.
  </Guideline.Do>
  <Guideline.Dont>
