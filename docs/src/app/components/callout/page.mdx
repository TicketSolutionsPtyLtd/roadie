...
`subtle` is the default. Use `normal` to sit on a tinted surface, `strong` for the one message a page can't miss, and `subtler` for a quiet aside.
...
<Callout intent='danger'>
  ...
  <Callout.Actions>
    <Button size='sm' emphasis='strong'>Update card</Button>
    <Button size='sm' emphasis='subtler'>View booking</Button>
  </Callout.Actions>
</Callout>
```
...
Actions sit below the text in a narrow callout and move beside it once the callout is 32rem wide. The callout measures itself, so it lays out the same in a sidebar as on a phone.
