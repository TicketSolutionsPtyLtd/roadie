export const metadata = {
  title: 'Fieldset',
  description: 'A compound wrapper for grouping related form fields with a legend',
  status: 'beta',
  category: 'Forms',
}

import { PropsDefinitions } from '@/components/PropsDefinitions'

# Fieldset

A compound wrapper for grouping related form fields with a legend.

## Import

```tsx
import { Fieldset } from '@oztix/roadie-components/fieldset'
```

`<Fieldset>` is the root component. Sub-components are attached as properties:
`<Fieldset.Legend>`, `<Fieldset.HelperText>`, `<Fieldset.ErrorText>`. For
consumers who prefer Base UI's explicit root syntax, `<Fieldset.Root>` is a
supported alias — the two forms reference the same function.

## Examples

### Default

```tsx-live
<Fieldset>
  <Fieldset.Legend>Contact information</Fieldset.Legend>
  <Field>
    <Field.Label>Name</Field.Label>
    <Field.Input placeholder='Your name' />
  </Field>
  <Field>
    <Field.Label>Email</Field.Label>
    <Field.Input type='email' placeholder='you@example.com' />
  </Field>
</Fieldset>
```

### Composition

Fieldset exposes sub-components for legend, helper text, and error text.

#### With helper text

```tsx-live
<Fieldset>
  <Fieldset.Legend>Contact information</Fieldset.Legend>
  <Fieldset.HelperText>Provide your preferred contact details.</Fieldset.HelperText>
  <Field>
    <Field.Label>Name</Field.Label>
    <Field.Input placeholder='Your name' />
  </Field>
  <Field>
    <Field.Label>Email</Field.Label>
    <Field.Input type='email' placeholder='you@example.com' />
  </Field>
</Fieldset>
```

#### With error text

```tsx-live
<Fieldset invalid>
  <Fieldset.Legend>Preferences</Fieldset.Legend>
  <Fieldset.ErrorText>Please select at least one option.</Fieldset.ErrorText>
</Fieldset>
```

<PropsDefinitions componentPath='packages/components/src/components/Fieldset' />
