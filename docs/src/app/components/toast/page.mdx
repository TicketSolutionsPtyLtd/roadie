...
// next/link takes more props than Roadie needs, so cast it once.
const Link = NextLink as unknown as RoadieLinkComponent
...
When the action is the only way forward, such as Retry, set `timeout: 0` so the toast waits for them. If the problem needs more than a retry, show a [Callout](/components/callout) next to it instead.
...
If the work fails because something needs fixing, such as missing event details, show a [Callout](/components/callout) next to the problem instead. It stays until the problem is fixed.
