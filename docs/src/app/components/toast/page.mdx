...
`RoadieProvider` mounts `Toast.Provider` and a `Toast.Viewport` at your app root, so toasts work once you've followed [Installation](/overview/getting-started#4-mount-roadieprovider). The viewport portals to the end of the page. Toasts stack at the bottom end of wide screens and span the bottom of small ones. See [Position](#position) to change that.
...
In a [Navigator](/components/navigator) app you don't set the top offset yourself. The frame sets it to the bottom of the tallest [Pane.Header](/components/pane) along the top of the window, so top toasts sit just below the header and follow it as it collapses.
