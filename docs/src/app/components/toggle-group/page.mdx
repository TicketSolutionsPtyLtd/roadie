...
A sunken track with a raised pill on the pressed item. The pill slides when the choice changes. Segments share the width of the widest.
...
Give an icon-only item an `aria-label`. It renders square.
...
- **Keyboard:** `Tab` moves into the group and out again. Arrow keys move between items (`ArrowLeft` and `ArrowRight`, or `ArrowUp` and `ArrowDown` when vertical), and `Home` and `End` jump to the ends. `Space` and `Enter` press the focused item.
