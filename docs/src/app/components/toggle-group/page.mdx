...
A tinted track with a raised pill on the pressed item, like Tabs. The pill slides when the choice changes. Segments share the width of the widest.
...
Emphasis sets the pressed pill. `strong` is solid and inverted, `normal` is raised and `subtle` is tinted, each in a tinted track. `subtler` is the only one without a track, for dense toolbars. The height is the same at every emphasis.
...
The intent tints the labels, the track and the pill's shadow.
...
- **Use `subtler` in a dense toolbar.** It has no track, so it sits quietly beside other controls. Save `strong` for a control that has to stand out.
