...
Emphasis sets the track at rest, as on `Toggle`, where pressed steps up to strong. `normal` has a bordered track and `subtle` a tinted one, each with a solid pill. `subtler` is the only one without a track, with a tinted pill, for dense toolbars. The height is the same at every emphasis.
