...
A raised track with a solid pill on the pressed item. The pill slides when the choice changes. Segments share the width of the widest.
...
Emphasis sets the track at rest, as on `Toggle`, where pressed steps up to strong. `normal` has a raised track and `subtle` a tinted one, each with a solid pill. `subtler` is the only one without a track, with a tinted pill, for dense toolbars. The height is the same at every emphasis.
