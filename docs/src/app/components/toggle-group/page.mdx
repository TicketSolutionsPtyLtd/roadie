...
Emphasis sets the track at rest, as on `Toggle`, where pressed steps up to strong. `normal` has a bordered track and `subtle` a tinted one, each with a solid pill. `subtler` is the only one without a track, for dense toolbars. Its pill is tinted with a strong edge and stronger text, so the pressed item stays clear even at `sm` with only an icon. The edge keeps at least 3:1 contrast against the surface in every intent, light or dark. The height is the same at every emphasis.
