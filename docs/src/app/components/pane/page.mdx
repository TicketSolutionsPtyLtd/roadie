...
inspector's `aria-label`, and keeps the page readable behind it. The drawer is a fixed `lg` sheet, so filtering its content can't resize it. Pass `drawerSize` to pick another [Drawer size](/components/drawer#sizes).
