...
animates like one that was already there. Swapping a sibling cuts: replacing a
pane with another at the same depth leaves the stack the shape it was, so it is
not a push. A first paint never slides. Columns
