...
Portaled popups sit outside the trigger's CSS cascade, so set the colour context directly on `Popover.Content` with the `intent` prop. Each intent fits a different message:

- `info` for inline help
- `success` for confirmation
- `warning` and `danger` for caution and destructive actions
- `accent` to highlight something new
...
function IntentExamples() {
  ...
  return (
    <div className='flex flex-wrap items-start gap-3'>
      ...
      {/* Success copies a link, confirms, then auto-dismisses after 2s */}
      <Popover open={copied} onOpenChange={copyLink}>
        ...
        <Popover.Content intent='success' side='top'>
          <Popover.Arrow />
          <Popover.Body>
      ...
      {/* Warning for caution before leaving */}
      <Popover>
      ...
      {/* Danger for a destructive confirm on an icon button */}
      <Popover>
      ...
      {/* Accent to highlight a new feature */}
      <Popover>
...
A popover opens below its trigger by default and re-flips to stay in view. Set these props on `Popover.Content` to control placement:

- `side` (`top`/`right`/`bottom`/`left`)
- `align` (`start`/`center`/`end`)
- `sideOffset`, the gap from the trigger in px
- `alignOffset`, the shift along the trigger's edge in px
...
Set `openOnHover` on `Popover.Trigger` (with an optional `delay`) so the panel opens on hover as well as click. Use it for previews and inline help. It still opens on focus and click, so keyboard users aren't locked out.
...
A `Popover.Footer` holds a centered actions row. It suits a lightweight confirm-deletion flow anchored to the trigger.
...
Use a popover for contextual content anchored to a control, such as menus, quick forms, and confirmations. It has no `size` prop and sizes to its content and anchor. Set `intent` on `Popover.Content`, not the trigger, since portaled popups do not inherit the trigger's CSS cascade.
...
<Guideline title='Reach for a dialog when the task blocks'>
  <Guideline.Do>
    Use a popover for contextual content anchored to a control. That covers menus, quick confirms, and lightweight panels that sit beside the trigger without taking over the page.
  </Guideline.Do>
  <Guideline.Dont>
...
<Guideline title='Anchor to the side that fits the action'>
  <Guideline.Do>
    Place the panel where it relates to the trigger. Put inline help to the `right` of a field, and a brief confirmation `top` of the button that fired it. Let it re-flip to stay in view.
  </Guideline.Do>
  <Guideline.Dont>
...
<Guideline title='Reserve hover for low-stakes content'>
  <Guideline.Do>
    Use `openOnHover` for previews and inline help that carry no decision, such as profile cards, definitions, and field hints. It still opens on focus and click, so keyboard users aren't excluded.
  </Guideline.Do>
  <Guideline.Dont>
...
- **Icon-only close**: When the dismiss control shows only an icon, give it an explicit label so screen readers announce its purpose. For example, `<Popover.Close render={<IconButton aria-label='Close'><XIcon weight='bold' /></IconButton>} />`.
