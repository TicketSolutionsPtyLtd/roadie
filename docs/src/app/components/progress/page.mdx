...
Reaching 100% only means the bar has finished. Wait until the app knows how the task ended, then set `intent` and say the result in `valueText`. A failed task stays where it stopped, with a [Callout](/components/callout) that explains what went wrong and offers a way to try again.
...
<Guideline title="Don't rely on colour alone">
  <Guideline.Do>
    Say Done or Failed in the label or `valueText`. For a failure, put a [Callout](/components/callout) next to the bar with the reason and a Retry action.
  </Guideline.Do>
  <Guideline.Dont>
