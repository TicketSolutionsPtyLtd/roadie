...
A bar that shows how far a task has got while it runs, built on Base UI's progress primitive.
...
<Guideline title='Progress for a task, Meter for a measure'>
  ...
  <Guideline.Dont code={`<Progress value={1842} max={2400} label='Tickets sold' />`}>
    Don't use Progress for an amount against a limit, such as capacity or tickets sold. Nothing is running, so use <code>Meter</code>.
  </Guideline.Dont>
</Guideline>
