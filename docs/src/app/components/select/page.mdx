...
Set `multiple` to let people pick more than one option. The popup stays open
between picks, and the trigger lists every label. `value`, `defaultValue` and
`onValueChange` then take an array.

```tsx-live
<Field>
  <Field.Label>Bands to see</Field.Label>
  <Select multiple defaultValue={['bee-gees', 'custard']}>
    <Select.Trigger>
