...
`Carousel.Title` accepts `as` so you can pass a framework router link (`Link` here) instead of a plain `<a>` — the trailing arrow icon and link styling come along automatically.
...
>
  <Carousel.Header className='px-4 sm:px-6'>
    <Carousel.Title as={Link} href='/events'>
      Featured events
    </Carousel.Title>
    <div className='flex items-center gap-2'>
      <Carousel.PlayPause />
