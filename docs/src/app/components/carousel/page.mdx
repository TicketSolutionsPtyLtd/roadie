...
The simplest usage — a single Content with Items, plus Previous/Next nav buttons.
...
Auto-rotating hero carousel with a persistent pause control. The `Carousel.PlayPause` button is placed first inside the header so it is the first tab stop — required by WCAG 2.2.2.
