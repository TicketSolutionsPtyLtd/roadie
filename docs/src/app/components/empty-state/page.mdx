...
Communicates that a section or page has no content. A single `size` token
scales it from a small empty section up to a whole-page empty or 404 screen.
...
The standalone `md` empty state has an IconTile, a message, and one action.
...
- `EmptyState.Title` renders an `<h2>` by default. Use `render={<h1 />}` (or
  ...
  A whole-page state usually wants `h1`.
