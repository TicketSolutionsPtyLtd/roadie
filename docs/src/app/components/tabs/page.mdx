...
A flat row with a sliding underline. Use for in-page section navigation where tabs should defer to the surrounding content. The underline takes the intent's strong colour, which in the default neutral intent is far above the 3:1 a selected mark needs, so it stays clear on an icon-only tab too. Under forced colours it takes the system highlight colour.
