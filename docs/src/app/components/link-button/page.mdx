...
- Use LinkButton for navigation that should look like a button (e.g., call-to-action links).
- Use [Button](/components/button) for actions that don't navigate (form submissions, toggles, dialogs).
- For icon-only links, use [LinkIconButton](/components/link-icon-button).
