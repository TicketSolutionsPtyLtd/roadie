...
Inside [Field](/components/field), leave out `label`. The slider takes its name from `Field.Label`, its description from the helper or error text, and `invalid` and `disabled` from the field.
