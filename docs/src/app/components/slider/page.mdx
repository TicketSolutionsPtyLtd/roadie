...
Inside [Field](/components/field), pass `label` to the slider. It stands in for `Field.Label`, so the label and value share a line above the track. The slider takes its description from the helper or error text, and `invalid` and `disabled` from the field. Clicking the label focuses the first thumb.
