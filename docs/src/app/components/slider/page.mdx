...
A track with a thumb to drag for a number or a range, built on Base UI's slider primitive.
...
Inside [Field](/components/field), leave out `label` and compose the parts so `Slider.Value` still shows the value. The slider takes its name from `Field.Label`, its description from the helper or error text, and `invalid` and `disabled` from the field. Clicking the label focuses the first thumb.
...
Set `orientation='vertical'` for a slider that runs up the page.

```tsx-live
<Slider aria-label='Stage monitor level' orientation='vertical' defaultValue={70} />
```
...
- The thumb looks 20px across but takes presses 44px across, and the whole track takes a press to jump there.
