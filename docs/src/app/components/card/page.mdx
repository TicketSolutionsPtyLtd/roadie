...
`variant` sets the card's shape. `default` is a plain card and applies when you leave the prop off. `ticket` cuts a notch into each side where the body meets `Card.Footer`, with a perforated line between them. The notches are real holes, so whatever sits behind the card shows through. Use `ticket` with `raised`, `normal` or `subtle` emphasis.
