...
`Card.Image` is an [`Image`](/components/image): pass `width` (and `height`) to serve a right-sized WebP instead of the original, and `placeholder='blur'` for a blur-up while it loads.
