...
Size a Skeleton the way you would size the content it stands in for. A `text` line tracks the type size around it, so wrap it in the element that carries the class.

```tsx-live
<div className='grid gap-4'>
  <p className='text-sm'><Skeleton /></p>
  <p className='text-lg'><Skeleton className='w-2/3' /></p>
...
The region that owns the fetch announces the wait. Put `aria-busy='true'` on it while the request is in flight and give it an accessible name, then let the result land in the same region.
...
The pulse runs on the `animate-pulse-subtle` utility from the motion tokens. Under `prefers-reduced-motion: reduce` the global motion reset cuts it to a single pass, so the placeholder holds a static tint and never flickers.
