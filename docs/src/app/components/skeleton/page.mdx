...
The motion runs on the `animate-shimmer` utility from the motion tokens: a tint pulse on `--duration-ambient` under a highlight that crosses on `--duration-sweep`. The highlight is lighter than the surface it crosses in both themes, from the `--sheen-shade` and `--sheen-highlight` tokens. Under `prefers-reduced-motion: reduce` the highlight is removed and the global motion reset cuts the pulse to a single pass, so the placeholder holds a static tint and never flickers.
