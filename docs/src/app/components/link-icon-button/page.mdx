...
> **Recommended:** wire [`RoadieLinkProvider`](/foundations/linking) at the app root and use [`<IconButton href={…}>`](/components/icon-button) — it routes through your configured Link automatically. The `as` prop below is preserved for back-compat only and will be removed in v3.0.0.
...
- Use [`<IconButton href={…}>`](/components/icon-button) for new navigation links — `LinkIconButton` is deprecated.
- Always provide an `aria-label` to describe the action since there is no visible text.
- For links with visible text, use [`<Button href={…}>`](/components/button) instead.
