...
Ark UI's stepper state, re-exported so `Steps.RootProvider` can take a value
you control. `props` takes the same `step` / `defaultStep` / `onStepChange` /
`linear` / `count` / `orientation` as `Steps` itself. The fields most calls
reach for:

| Field | Type | Description |
| --- | --- | --- |
| `value` | `number` | The current step index. |
| `count` | `number` | The total number of steps. |
| `hasNextStep` | `boolean` | Whether there is a step after this one. |
| `hasPrevStep` | `boolean` | Whether there is a step before this one. |
| `isCompleted` | `boolean` | Whether the stepper has finished. |
| `setStep` | `(step: number) => void` | Jump to a step. |
| `goToNextStep` | `() => void` | Advance one step. |
| `goToPrevStep` | `() => void` | Go back one step. |
| `resetStep` | `() => void` | Return to the first step. |

<PropsDefinitions componentPath='packages/components/src/components/Steps' />
