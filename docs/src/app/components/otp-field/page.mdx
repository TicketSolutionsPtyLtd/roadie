...
A one-time code input with one slot per character, built on Base UI's OTP field primitive.
...
`md` slots are 48px square, a comfortable touch target. When the row runs out of room, the slots shrink together rather than wrap.
