...
<Guideline
  ...
  description="Base UI watches ScrollArea.Content with a ResizeObserver. The viewport is only observed for its own box, which never changes. Without the wrapper, an area whose content grows or shrinks keeps whatever overflow state it started with."
>
