...
<div className='w-72'>
  <DataCard
    ...
    actions={<Button size='sm' emphasis='subtle'>View chart</Button>}
  />
</div>
```
...
- Pass `actions` for the one or two things a person can do from the card, such as switching to a table view.

## Accessibility

- The card is a region named by its label, and truncated label or context text stays available in its `title` attribute.
- `state='loading'` sets `aria-busy`, so assistive tech knows the card isn't ready.

<PropsDefinitions componentPath='packages/components/src/components/DataCard/index.tsx' />
