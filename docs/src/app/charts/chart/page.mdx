...
- Actions sit after the view switch and stay in every state, so a refresh still works on a card that failed to load. The view switch shows only when there's data. Beside More, keep chart labels to 18 characters at `sm` and `md` and 23 at `lg` and `full`. See [card actions](/charts/dashboards#card-actions).
