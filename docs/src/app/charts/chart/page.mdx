    ...
    legend={
      <ChartLegend
        items={[
          ...
          { label: 'Similar shows median', shape: 'dash', color: 'var(--chart-median)' },
...
Pass `actions` to add controls after the Chart and Table switch. Use at most one visible action, then `DataCard.MoreButton` last. It passes its props and ref through, so it becomes the trigger of Roadie's upcoming Menu component with no change.
