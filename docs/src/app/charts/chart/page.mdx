    ...
    legend={
      <ChartLegend
        items={[
          ...
          { label: 'Similar shows', shape: 'band', color: 'var(--chart-band)', median: 'var(--chart-median)' },
...
Pass `actions` to add controls after the Chart and Table switch. Use at most one visible action, then the More menu last. Pass `DataCard.MoreButton` to a [Menu](/components/menu) trigger's `render`, and it keeps its name and ref. Group the items as the [More menu](/charts/dashboards#card-actions) guideline says, with the danger action last.
...
- The switch is a toggle group, not tabs, because Chart and Table are two views of the same data. See [Tabs switch panels; a toggle group sets a value](/components/tabs#guidelines).
...
- Actions sit after the view switch and stay in every state, so a refresh still works on a card that failed to load. The view switch shows only when there's data. Beside More, keep chart labels within `ACTIONS_LABEL_LIMITS` from `@oztix/roadie-core/dashboard-layout`. See [card actions](/charts/dashboards#card-actions).
