...
A single line gets no legend of its own on a wide card, where end labels name its reading aids. Pass one when you want the band and the line named at every width.
