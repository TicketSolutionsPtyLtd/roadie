...
Roadie's core CSS layer works with any framework. It covers tokens, intents, emphasis, elevation, typography, and interaction utilities. This guide covers using Roadie in a Vue application where only the CSS foundation is needed.
...
Toggle dark mode by adding the `dark` class to the `<html>` element. All tokens adapt automatically, so you need no `dark:` variants.
...
Everything else works identically. That includes tokens, intents, emphasis, elevation, typography, interactions, motion, and all Tailwind utilities.

## Next steps

- Browse the [Foundations](/foundations) pages for detailed guidance on each system
...
- Check the [Installation](/overview/getting-started) guide for React setup
