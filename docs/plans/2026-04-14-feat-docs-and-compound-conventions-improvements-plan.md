...
| **2.5 — `/parts` subpath export (follow-up)**     | 🚫 Superseded | —                                                              | Superseded by Phase 3 of [`2026-04-15-refactor-components-consistency-cleanup-plan.md`](./2026-04-15-refactor-components-consistency-cleanup-plan.md). Rather than ship a dedicated `/parts` subpath, Phase 3 rebuilt every compound on a per-file leaf layout under tsdown `unbundle: true` with a **server-safe `index.tsx` property-assignment layer** (no `'use client'` directive). The compound's `index.tsx` is now an ordinary server module that Next.js can follow through to each leaf's client boundary, so `<Compound.Sub>` dot-access works from server components via both the per-compound subpath (`@oztix/roadie-components/<compound>`) and the root barrel — no separate `/parts` entry point required. `docs/src/components/PropsAccordion.tsx` was deleted in commit `805e6c5` as the concrete green signal. |
